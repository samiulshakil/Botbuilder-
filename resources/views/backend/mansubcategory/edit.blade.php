    ...
    <div class="card">
        <div class="card-body">
            <div class="card-title">
                <form action="{{route('mansubcategories.update', $subCategory->id)}}" method="POST" enctype="multipart/form-data">
                    @csrf 
                    @method('PUT')
                        <div class="mb-3">
                            ...
                            <select class="form-select form-control" name="mancategory_id" aria-label="Default select example">
                                ...
                                @foreach ($categories as $category)
                                    <option {{$subCategory->mancategory_id == $category->id ? 'selected="selected"' : ''}} value="{{ $category->id }}">{{$category->name}}</option>
                                @endforeach
                            </select>
                        </div>
                    <div class="mb-3">
